<template>
  <div id="app" class="container">
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <tr v-for="(sc, index) in sportCultureNews" :key="index">
          <td>{{ sc.scId }}</td>
          <td>{{ sc.title }}</td>
          <td>{{ sc.description }}</td>
          <td>{{ sc.dateUpload }}</td>
          <td>
            <img :src="`http://localhost:9513/photos/${sc.img}`" style="width: 150px; height: 100px;" />
          </td>
          <td>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal"
              style="margin-right: 5px;" @click="editClick(sc)">
            ...
            <button type="button" class="btn btn-danger" @click="deleteClick(sc.scId)">
          ...
          <div class="modal-body">
            <div class="d-flex flex-row bd-highlight mb-3">
              <div class="p-2 w-50 bd-highlight">
                ...
                <div class="input-group mb-3"  style="height: 175px;">
              ...
              <div class="p-2 w-50 bd-highlight">
                <img v-if="scId != 0" :src="`http://localhost:9513/photos/` + img"
            ...
            <button type="button" v-if="scId == 0" class="btn btn-primary" @click="createClick()">Create</button>
            <button type="button" v-if="scId != 0" class="btn btn-primary" @click="updateClick()">Update</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    refreshedData () {
      axios.get('http://localhost:9513/api/sportCultureNews').then((response) => {
        this.sportCultureNews = response.data
      })
    },
    addClick () {
      this.modalTitle = 'Add sportCultureNews'
      this.scId = 0
      this.title = ''
      this.origin = ''
      this.url = 'SpCulNewsDetail'
      this.dateTimeUpload = ''
      this.views = 0
      this.sport = 'true'
    ...
    createClick () {
      axios.post('http://localhost:9513/api/sportCultureNews', {
        title: this.title,
        origin: this.origin,
        url: this.url,
    ...
    editClick (sc) {
      this.modalTitle = 'Edit sportCultureNews'
      this.scId = sc.scId
      this.title = sc.title
      this.origin = sc.origin
      this.url = sc.url
      this.dateTimeUpload = sc.dateTimeUpload
      this.views = sc.views
      this.author = sc.author
      this.note = sc.note
      this.sport = sc.sport
      this.rest = sc.rest
      this.description = sc.description
      this.dateUpload = sc.dateUpload
      this.img = sc.img
    },
    updateClick () {
      axios.put('http://localhost:9513/api/sportCultureNews', {
        scId: this.scId,
        title: this.title,
        origin: this.origin,
        url: this.url,
    ...
    deleteClick (id) {
      ...
      axios.delete('http://localhost:9513/api/sportCultureNews/' + id)
        .then((response) => {
          this.refreshedData()
          alert(response.data)
        })
    ...
    imageUpload (event) {
      ...
      axios.post(
        'http://localhost:9513/api/sportCultureNews/SaveFile', formData
      ).then((response) => {
